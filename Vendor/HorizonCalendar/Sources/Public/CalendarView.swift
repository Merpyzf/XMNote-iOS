...
// MARK: - HorizontalMonthPagingProgressContext

/// The horizontal paging progress between two adjacent months.
public struct HorizontalMonthPagingProgressContext: Hashable {

  // MARK: Public

  /// The month at the leading edge of the viewport at the start of interpolation.
  public let fromMonth: MonthComponents

  /// The next month at the leading edge of the viewport at the end of interpolation.
  public let toMonth: MonthComponents

  /// The normalized interpolation progress between `fromMonth` and `toMonth` in `[0, 1]`.
  public let progress: CGFloat

  /// Whether the user is currently dragging the scroll view.
  public let isUserDragging: Bool
}
...
public final class CalendarView: UIView {
  ...
  /// A closure (that is retained) that is invoked while a horizontally-paginated calendar scrolls between adjacent months.
  public var didScrollHorizontalMonthPagingProgress: ((HorizontalMonthPagingProgressContext) -> Void)?
  ...
  fileprivate func horizontalMonthPagingProgressContext(
    isUserDragging: Bool
  ) -> HorizontalMonthPagingProgressContext? {
    guard content.monthsLayout.isHorizontal else { return nil }
    guard let visibleItemsDetails else { return nil }

    let sortedVisibleMonths = visibleItemsDetails.framesForVisibleMonths
      .map { (month: $0.key, minX: $0.value.minX) }
      .sorted { $0.minX < $1.minX }

    guard sortedVisibleMonths.count >= 2 else { return nil }

    let leadingEdgeX = scrollView.contentOffset.x
    var from = sortedVisibleMonths[0]
    var to = sortedVisibleMonths[1]

    if leadingEdgeX >= sortedVisibleMonths[sortedVisibleMonths.count - 1].minX {
      from = sortedVisibleMonths[sortedVisibleMonths.count - 2]
      to = sortedVisibleMonths[sortedVisibleMonths.count - 1]
    } else if leadingEdgeX > sortedVisibleMonths[0].minX {
      for index in 0..<(sortedVisibleMonths.count - 1) {
        let candidateFrom = sortedVisibleMonths[index]
        let candidateTo = sortedVisibleMonths[index + 1]
        if leadingEdgeX >= candidateFrom.minX, leadingEdgeX <= candidateTo.minX {
          from = candidateFrom
          to = candidateTo
          break
        }
    ...
    let distance = to.minX - from.minX
    guard distance > 0 else { return nil }

    let rawProgress = (leadingEdgeX - from.minX) / distance
    let progress = min(1, max(0, rawProgress))

    return HorizontalMonthPagingProgressContext(
...
private final class ScrollViewDelegate: NSObject, UIScrollViewDelegate {
  ...
  func scrollViewDidScroll(_ scrollView: UIScrollView) {
    ...
    if let context = calendarView.horizontalMonthPagingProgressContext(
      isUserDragging: isUserInitiatedScrolling)
    {
      calendarView.didScrollHorizontalMonthPagingProgress?(context)
    }

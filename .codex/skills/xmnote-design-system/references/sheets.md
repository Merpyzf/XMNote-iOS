...
- Apple API、可用性与系统行为遵守 `AGENTS.md` 的查证顺序及官方来源降级规则；已取得且仍适用于当前问题和版本的证据可以复用，本参考不替代当前 SDK 文档。
...
`WereadImportActionPanel` 是依附授权网页、可收起且底部主操作持续可见的功能控制层。它不是独立的短时编辑任务 Sheet，不为套用普通骨架而补一套标题栏、关闭按钮、Settings 卡片或导航栈；退出与授权反馈仍归现有呈现宿主。

- 内容中的“书籍范围”等是设置标签，不视为 Sheet 标题。本案例按用户确认使用 `AppTypography.body` 的 Regular 字重降低长中文标签的视觉重量；范围值与说明继续使用 Settings 排版，全局 `SettingsTypography.rowTitle` 的 Medium 默认不变。
- 阅读时长说明在左侧文本列与标题成组，右侧原生开关对齐标题；左列可点击切换，辅助技术仍由原生开关提供名称和状态，避免重复朗读装饰性文本。
- 展开态验证说明换行后与固定主按钮不重叠；关闭说明不应使固定按钮跳动。收起态只保留应可操作的内容，隐藏设置不能残留可点击或可访问节点。
- 收起和展开都要等待手势结束后检查截图，分别核对拖拽条、按钮与底部留白；“按钮可点击”不能替代高度验收。隐藏内容仍可能参与布局，系统 Detent、实时内容尺寸与安全区也不一定相等；调整前先测量真实边界，避免固定高度与手动位移重复补偿，也不要直接用按钮高度推导所有系统 Sheet 的最小高度。
- 验证默认/辅助字号、紧凑宽度、深浅色以及加载/禁用；大字号需要可滚动内容和可达主操作，不能仅把普通字号截图当作覆盖。
- 本案例的主操作使用“获取书籍”，获取阶段不提前表述为导入。数量已知时使用“获取中 3/100”，末尾不加省略号；未知数量或空间不足时使用“获取中…”。数字过渡只作用于数量，任务切换和减弱动态效果下不滚动；不通过增加底部行、缩字或修改 Detent 容纳长进度。

2026-09-06 的局部验证覆盖该面板默认 402pt、临时 320pt 窗口及最大辅助字号：左列说明可换行，开关对齐标题，主操作保留独立空间。`Spacing.half` 的文字组间距和现有 Detent 高度仅说明这个案例，不规定所有 Sheet 的固定布局，也不授予该业务 View 公共组件身份。
...
按钮材质与按压反馈按 [按钮样式与主操作](buttons-and-primary-actions.md) 选择。顶部确认、取消继续由 scaffold 和系统 toolbar 持有外观，不因项目采用交互式玻璃主按钮就额外包玻璃或迁移其位置。
...
- 已按上表确定需要 `bottomBar`，且按钮属于独立功能控制层时，按 [按钮样式与主操作](buttons-and-primary-actions.md) 默认采用交互式 Liquid Glass；已有 canonical 专项按钮仍遵守其 owner。不为展示新材质新增底栏，也不把普通表单行或内容尾部按钮自动视为浮动主操作。

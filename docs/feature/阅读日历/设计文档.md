...
- 文件：
  ...
  - `xmnote/Data/Repositories/ReadCalendarColorRepository.swift`
  - `xmnote/Domain/Repositories/RepositoryProtocols.swift`
- 职责：
  - 提供最早可用日期、指定月份聚合数据。
  - 提供阅读事件条封面取色（dominant + 文本可读性）能力。
- 接口契约：
  ...
  - `resolveEventColor(bookId:bookName:coverURL:) async -> ReadCalendarSegmentColor`

### 2.3 ViewModel 层
- 文件：
  - `xmnote/Views/Reading/ReadCalendar/ReadCalendarViewModel.swift`
  - `xmnote/Views/Reading/ReadCalendar/ReadCalendarEventLayoutEngine.swift`
- 职责：
  - 状态中枢：分页、选中态、错误态、缓存、竞态控制。
  - 状态中枢：分页、选中态、错误态、缓存、竞态控制、取色任务编排。
...
- 并发状态：
  ...
  - `colorTicketSeed: Int`
  - `latestColorTicketByMonthKey: [String: Int]`
  - `monthColorTasks: [String: Task<Void, Never>]`
...
- `buildWeeks(monthStart:dayMap:)`
  - 周网格与事件段合并的纯计算函数。
- `scheduleColorResolutionIfNeeded(for:using:)`
  - 从 pending segments 中构建去重请求并启动异步取色。
- `resolveSegmentColors(...)`
  - 增量回填 segment 颜色，旧 ticket 结果丢弃。
- `cancelOutOfScopeColorTasks(around:)`
  - 切月后仅保留当前月与相邻月任务，避免无关任务串月回写。
...
- `ReadCalendarMonthGrid` 内部周行负责单周网格与 segment overlay：
  ...
  - segment 颜色三态：
  - `pending`：骨架底 + shimmer
  - `resolved`：封面 dominant 色
  - `failed`：哈希回退色

## 6.5 封面取色策略（对齐 Android 业务意图）
1. 取色目标：封面图片中**像素占比最大的颜色**（dominant）。
2. 取色中：不显示哈希色，统一使用低显色骨架 + shimmer，明确“正在取色”语义。
3. 取色成功：渲染 dominant 背景色，并动态计算文本色。
4. 取色失败：仅失败时回退哈希色。
5. 缓存：
   - 内存缓存：页面内与会话内快速复用。
   - 磁盘缓存：跨页面/冷启动复用，避免重复下载与重复分析。

### 6.4 视觉系统（纸感 + 轻拟物）
- 背景：二级页面统一 `Color.windowBackground`，不叠加首页顶部渐变阴影。
- 主卡：统一大圆角（`CornerRadius.containerMedium`）+ 克制阴影，形成“浮起但不抢眼”的层级。

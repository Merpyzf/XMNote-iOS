...
备份包允许携带平台专属附加层。数据库仍是跨端事实源；`xmnote_ios_preferences.json` 只服务 iOS 设备间设置恢复，Android 无需读取，也不得把它纳入 Room schema 合同。
...
- iOS 偏好 sidecar 缺失或失败时不影响数据库恢复，也不改变 Android 的归档解析要求。
...
| iOS 偏好 sidecar | ZIP 中可选的 `xmnote_ios_preferences.json`，只包含类型化白名单设置，仅由 iOS 读取。 |
...
`xmnote/Services/BackupService.swift` 负责统一归档 I/O、解压、版本读取、正式替换、回滚与平台 sidecar 的结构判定；它不直接理解 Room schema 或偏好业务字段。恢复前的数据库结构与数据整理统一交给 `BackupSchemaValidator.prepareForRestore`，iOS 偏好字段映射统一交给 `IOSUserDefaultsBackupCoordinator`。
...
- 备份包可包含 iOS 专属 sidecar；Android 必须把它视为可忽略的附加文件。

Android 端恢复时应先做版本与 Room schema 校验，再替换正式库。若发现高版本备份或 schema 不一致，应阻断恢复并保留 Android 本机库。

### 7.3 iOS 平台偏好附加层

iOS 生成归档时在数据库与 Android 兼容 XML 之外写入可选的 `xmnote_ios_preferences.json`。该文件采用编译期类型化白名单，当前只包含完整 AI 配置；不遍历整个 UserDefaults，也不要求 Android 消费。

iOS 恢复时先完成 staging 校验、数据库替换与连接重开，再应用偏好快照。无 sidecar 的 Android/旧 iOS 备份保持当前设备设置；sidecar 损坏或版本过高只返回“部分设置未能恢复”，不得回滚或牺牲已经恢复的数据库。详细格式、版本和扩展准入见 `docs/architecture/数据备份机制设计.md`。
...
- tombstone 不应进入书架、书籍工作台、统计、标签、分组等 UI 业务结果。
...
| iOS sidecar 缺失 | 数据库照常恢复 | 数据已恢复 |
| iOS sidecar 损坏或版本过高 | 数据库照常恢复，跳过偏好 | 数据已恢复，部分设置未能恢复 |
...
- iOS sidecar 只包含白名单设置；Android 备份无 sidecar 时 iOS 数据库恢复不受影响。

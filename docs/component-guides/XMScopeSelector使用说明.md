...
- 角色：同一内容集合内的单选范围切换控件，用统一胶囊、滑动指示器、数量 badge 与跟手拖拽承载 2-5 个互斥选项。
- 边界：组件只负责范围选项展示、选中态写回、点击/拖拽交互、Dynamic Type 与无障碍语义；不负责搜索执行、数据筛选、数量统计或业务路由。
- 验证状态：当前仅在 Debug 测试中心验证，尚未接入 `GlobalSearchView`、`BookPickerView`、`BookSearchView` 等生产页面。
...
| 参数 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `items` | `[XMScopeSelectorItem<ID>]` | 必填 | 2-5 个等宽互斥选项。Debug 下非法数量会断言，生产下渲染为空，避免错误 UI 混入页面。 |
...
## 常见问题
### 1) 为什么只支持 2-5 项？
这是语义和可读性边界。1 项不构成范围切换，超过 5 项后等宽分段会让标题和数量互相挤压，应改用菜单、列表筛选或搜索条件面板。
...
### 5) 接入生产页面前要检查什么？
先确认场景语义是“同一内容集合内的互斥范围切换”，并准备 2-5 个稳定选项。接入前应在测试中心验证浅色、深色、浮层、大字号、Reduce Motion 与 VoiceOver 读法，再替换真实页面里的临时按钮组。

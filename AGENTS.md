...
- 做事方法论（强制）：先定义问题层级，再选择解决层级；单点现象默认按单点问题排查，禁止在证据不足时直接上升为“架构问题”“框架缺能力”或“需要基建”。
- 事实验证优先（强制）：先验证事实，再解释原因，再设计方案；禁止先有结论、再补证据。
- 归因优先（强制）：在没有完成根因归因前，禁止做扩散式改造、全局抽象或跨页面推广；必须先找到真实状态/数据 owner，以及究竟是谁在改写状态。
- 抽象准入（强制）：只有当同一类问题在多个独立场景中被证明具备相同根因和相同修复模式后，才允许上升为公共方案或基础设施。
- 止损机制（强制）：一旦发现最初前提可能错误，必须立即收缩问题定义并重建判断，禁止边怀疑前提边继续推进大方案。
...
- 默认排查顺序（强制）：复现现象 → 确认真实 owner → 找到状态/数据写入点 → 区分实现问题、使用问题和平台行为 → 证明共性后再考虑抽象。
...
- 异步操作必须提供可感知反馈（加载态/按钮禁用/错误提示），避免“点击无响应”。
- 加载反馈分级（强制）：读取类加载采用“延迟显示 + 最短驻留”策略，默认阈值 `delay=150ms`、`minimumVisible=200ms`，防止瞬时闪烁。
- 写操作反馈分级（强制）：保存/删除/提交等写操作必须即时反馈，不做延迟显示；操作期间必须禁用重复触发入口。

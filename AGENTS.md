...
- 注释规范（强制）：新增/修改代码必须补充与复杂度匹配的高信息注释，优先说明“为什么这样做”，禁止无信息量注释与代码复述。
- UI 组件注释规范（强制）：`xmnote/UIComponents` 与 `xmnote/Views/<Feature>/Components` 的新增/重大修改组件，必须在组件定义处标明“组件作用”与“所属功能/使用场景”；关键参数需补充语义、边界与空态/异常策略说明。
- 复杂 UI 分支注释规范（强制）：存在多状态切换、布局技巧或交互约束时，必须使用简短分块注释标注意图，防止后续误改。

 ...
 * [OUTPUT]: 对外提供 XMNote 跨模块稳定语义颜色，包含中性炭灰深色表层、分级文字、品牌表面内容前景、搜索关键字、轻量状态动作与选择描边
...
extension Color {
    ...
    static let surfacePage = Color.xmAdaptive(
        light: Color.xmResolved(.systemGroupedBackground),
        dark: Color.xmHex(0x121212),
        highContrastLight: Color.xmResolved(.systemGroupedBackground),
        highContrastDark: Color.xmResolved(.systemGroupedBackground)
    )
    /// 默认内容卡片背景，承接页面底板上的主要内容容器。
    static let surfaceCard = Color.xmAdaptive(
        light: Color.xmResolved(.secondarySystemGroupedBackground),
        dark: Color.xmHex(0x242424),
        highContrastLight: Color.xmResolved(.secondarySystemGroupedBackground),
        highContrastDark: Color.xmResolved(.secondarySystemGroupedBackground)
    )
    /// 嵌套在主卡片上的次级表层，承接 Sheet 内局部模块或多层卡片结构。
    static let surfaceNested = Color.xmAdaptive(
        light: Color.xmResolved(.tertiarySystemGroupedBackground),
        dark: Color.xmHex(0x2C2C2C),
        highContrastLight: Color.xmResolved(.tertiarySystemGroupedBackground),
        highContrastDark: Color.xmResolved(.tertiarySystemGroupedBackground)
    )
    /// Sheet 根背景，和页面底板保持同一 grouped 语义。
    static let surfaceSheet = Color.xmAdaptive(
        light: Color.xmResolved(.systemGroupedBackground),
        dark: Color.xmHex(0x121212),
        highContrastLight: Color.xmResolved(.systemGroupedBackground),
        highContrastDark: Color.xmResolved(.systemGroupedBackground)
    ...
    static let controlFillSecondary = Color.xmAdaptive(
        light: Color.xmResolved(.tertiarySystemFill),
        dark: Color.xmHex(0x2C2C2C),
        highContrastLight: Color.xmResolved(.tertiarySystemFill),
        highContrastDark: Color.xmResolved(.tertiarySystemFill)
    )
...
extension Color {
    ...
    static let iconPrimary = Color.xmAdaptive(
        ...
        dark: Color.xmHex(0xD4D4D4),
        highContrastLight: Color.xmHex(0x000000),
        highContrastDark: Color.xmHex(0xFFFFFF)
    )
    /// 次级操作图标在深色中保持可操作性，浅色与高对比度继续使用原次要文字层级。
    static let iconSecondary = Color.xmAdaptive(
        light: textSecondary,
        dark: Color.xmHex(0xC2C2C2),
        highContrastLight: textSecondary,
        highContrastDark: textSecondary
    )
...
extension Color {
    /// 一级容器边框（页面主卡/分组壳层）
    static let surfaceBorderStrong = Color.xmAdaptive(
        light: Color.xmResolved(.opaqueSeparator),
        dark: Color.xmHex(0x484848),
        highContrastLight: Color.xmResolved(.opaqueSeparator),
        highContrastDark: Color.xmResolved(.opaqueSeparator)
    )
    /// 二级内容边框（指标卡/列表卡）
    static let surfaceBorderDefault = Color.xmAdaptive(
        light: Color.xmResolved(.separator),
        dark: Color.xmHex(0x383838),
        highContrastLight: Color.xmResolved(.separator),
        highContrastDark: Color.xmResolved(.separator)
    )
    /// 三级弱边框（弱化层级、避免与主信息竞争）
    static let surfaceBorderSubtle = Color.xmAdaptive(
        light: Color.xmResolved(.separator).opacity(0.72),
        dark: Color.xmHex(0x303030),
        highContrastLight: Color.xmResolved(.separator).opacity(0.72),
        highContrastDark: Color.xmResolved(.separator).opacity(0.72)
    )
...
extension Color {
    /// 承载成对中性前景的主提交表面；直接跟随品牌主题色，避免局部主操作出现独立色阶。

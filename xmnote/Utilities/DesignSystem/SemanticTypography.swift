 ...
 * [POS]: Utilities/DesignSystem 的语义排版桥接层，负责在不抬高默认视觉密度的前提下补齐语义缩放与最小可读字号约束
...
enum SemanticTypography {
    nonisolated static let minimumReadablePointSize: CGFloat = 11

    /// 返回保留 base size 的 SwiftUI 语义字体；未指定 traits 时保留环境依赖，继续响应 Dynamic Type。
    static func font(
        baseSize: CGFloat,
        ...
        guard let traitCollection else {
            let defaultPointSize = defaultPointSize(for: textStyle.uiFontTextStyle)
            return Font
                .system(textStyle, design: design, weight: weight)
                .scaled(by: clampedBaseSize / defaultPointSize)
        }
        ...
        let uiFont = uiFont(
            ...
            minimumPointSize: clampedBaseSize,
    ...
    nonisolated static func defaultPointSize(for textStyle: UIFont.TextStyle) -> CGFloat {

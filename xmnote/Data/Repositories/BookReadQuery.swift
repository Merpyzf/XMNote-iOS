...
/**
 * [INPUT]: 依赖 GRDB Database 与 book、note、chapter、attach_image、tag、tag_note、category、category_content、review、read_status、book_read_status_record、source、read_time_record 表读取书籍展示数据
 * [OUTPUT]: 对外提供 BookReadQuery（书籍卡片、书籍选择、书籍工作台资料与单书四域内容只读查询）
...
nonisolated enum BookReadQuery {
    ...
    /// 查询书籍下的书摘列表，供书籍工作台书摘域展示。
    /// - Throws: 数据库查询失败时抛出错误。
    static func fetchNotes(_ db: Database, bookId: Int64) throws -> [NoteExcerpt] {
        // SQL 目的：拉取书籍下的有效书摘并附带有效章节信息，供工作台按章节分组。
        // 涉及表：note n LEFT JOIN chapter c；n.chapter_id -> c.id。
        // 关键过滤：限定 book_id 且排除软删除 note。
        // 时间字段：created_date 为 Android 毫秒时间戳，按 DESC 排序；不做时区转换。
        // 返回字段用途：保留富文本内容、想法、位置与 include_time，供书籍工作台渲染。
        let sql = """
            ...
            ORDER BY c.parent_id ASC, c.chapter_order ASC, c.source_order ASC,
                     n.created_date DESC, n.id DESC
            """
    ...
    static func fetchRelated(_ db: Database, bookId: Int64) throws -> [BookRelatedExcerpt] {
        ...
        // 时间字段：created_date 为 Android 毫秒时间戳，不做时区转换；按分类顺序、记录 id 倒序排列。
        ...
        let sql = """
            ...
            LEFT JOIN book linked_book ON linked_book.id = cc.content_book_id AND linked_book.is_deleted = 0
            WHERE cc.book_id = ? AND cc.is_deleted = 0
            ORDER BY category_order ASC, cc.id DESC
            """

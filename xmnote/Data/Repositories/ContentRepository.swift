 ...
 * [OUTPUT]: 对外提供 ContentRepository（ContentRepositoryProtocol 的 GRDB/UserDefaults 实现），包含单书内容排序读写
 * [POS]: Data 层通用内容仓储实现，统一封装查看、编辑自动草稿、持久化排序、原子保存与硬删除事务
...
struct ContentRepository: ContentRepositoryProtocol {
    ...
    /// 物理删除当前书可管理的私有或全局自定义分类，并清理所有书中的子内容与占位引用。
    func deleteBookRelatedCategory(bookID: Int64, categoryID: Int64) async throws {
        ...
        try await databaseManager.database.dbPool.write { db in
                ...
                // SQL 目的：在关系删除前收集该分类引用的相关书籍主键。
                // 涉及表：category_content。
                // 关键过滤：category_id 精确命中且 content_book_id>0；全局分类会覆盖所有来源书，包含遗留删除标记关系。
                // 时间字段：不读取时间字段。
                // 返回字段用途：删除关系后逐一清理失去最后引用的占位书。
                let relatedBookSQL = """
                    SELECT DISTINCT content_book_id
                    FROM category_content
                    WHERE category_id = ? AND content_book_id > 0
                    """
                ...
                try db.execute(
                    // SQL 目的：先物理删除分类内全部相关内容附图，满足父内容删除前的 child-first 顺序。
                    // 涉及表：category_image，子查询 category_content。
                    // 关键过滤：子查询按 category_id 精确限定所属关系；全局分类会覆盖所有来源书，不依赖 is_deleted 状态。
                    // 时间字段：不读取时间字段。
                    // 副作用：清除普通内容和异常相关书籍关系可能残留的全部附图。
                    sql: """
                ...
                try db.execute(
                    // SQL 目的：物理删除自定义分类下的全部内容与相关书籍关系。
                    // 涉及表：category_content。
                    // 关键过滤：category_id 精确命中，不追加 is_deleted 条件以清理历史 tombstone。
                    // 时间字段：不读取时间字段。
                    // 副作用：移除分类主记录的全部业务子关系。
                    sql: "DELETE FROM category_content WHERE category_id = ?",
                    arguments: [categoryID]
                )
                try db.execute(
                    // SQL 目的：物理删除已清空的私有或全局自定义分类主记录。
                    // 涉及表：category。
                    // 关键过滤：主键已通过可管理 owner 与固定种子保护门闩。
                    // 时间字段：不读取时间字段。
                    // 副作用：删除分类本体。
                ...
                guard db.changesCount > 0 else { throw ContentRepositoryError.categoryNotFound }
                for relatedBookID in relatedBookIDs {
                    try deleteReferencePlaceholderIfUnreferenced(db, bookID: relatedBookID)
                }
        }
    }
        ...
        let savedReviewID = try await databaseManager.database.dbPool.write { db in
            var savedReviewID: Int64 = 0
                ...
                case .edit(let reviewID):
                    ...
                    try deleteReviewImages(db, reviewID: reviewID)
                    try db.execute(
        ...
        let savedContentID = try await databaseManager.database.dbPool.write { db in
            var savedContentID: Int64 = 0
                ...
                case .edit(let contentID):
                    ...
                    try deleteRelevantImages(db, contentID: contentID)
                    try db.execute(
...
private extension ContentRepository {
    ...
    /// 在更新书评主记录前物理删除全部旧图片，保证编辑事务采用 child-first 顺序。
    nonisolated func deleteReviewImages(_ db: Database, reviewID: Int64) throws {
        try db.execute(
            // SQL 目的：物理移除书评原有全部图片子记录，为全量有序替换建立干净集合。
            // 涉及表：review_image。
            // 关键过滤：按 review_id 精确命中，不保留 Android 历史 tombstone。
            // 时间字段：不读取或写入时间字段。
            // 副作用：仅删除图片关系，不删除远端对象，避免共享 URL 被误删。
    ...
    /// 在更新相关内容主记录前物理删除全部旧图片，保证编辑事务采用 child-first 顺序。
    nonisolated func deleteRelevantImages(_ db: Database, contentID: Int64) throws {
        try db.execute(
            // SQL 目的：物理移除普通相关内容原有全部图片子记录，为全量有序替换建立干净集合。
            // 涉及表：category_image。
            // 关键过滤：按 category_content_id 精确命中，不保留 Android 历史 tombstone。
            // 时间字段：不读取或写入时间字段。
            // 副作用：仅删除图片关系，不删除远端对象，避免共享 URL 被误删。
...
private extension ContentRepository {
    /// Android AppConstant.Categories.BOOK 的固定系统分类主键，只承载相关书籍关系。
    nonisolated static var relatedBookCategoryID: Int64 { 1 }
    ...
    /// 删除最后一个有效引用消失后的业务占位书，保留仍被书单或其他相关关系使用的记录。
    nonisolated func deleteReferencePlaceholderIfUnreferenced(
        ...
        try db.execute(
            // SQL 目的：物理删除失去最后一个真实引用的业务占位书。
            // 涉及表：book，反查 category_content 与 collection_book。
            // 关键过滤：只处理 is_deleted=1 且非系统根书；任一物理关系仍存在时保留。
            // 时间字段：不读取时间字段。
            // 副作用：最后引用移除后删除 book 主记录，不影响有效书籍。
            sql: """
                DELETE FROM book

 ...
 * [OUTPUT]: 对外提供 NoteRepository（NoteRepositoryProtocol 的 GRDB 实现），覆盖聚合列表、章节范围、批量写入、合并、编辑草稿与保存事务
 * [POS]: Data 层笔记仓储实现，统一收口书摘读取、child-first 硬删除、关系替换、跨书章节迁移与合并原子事务
...
struct NoteRepository: NoteRepositoryProtocol {
    ...
    /// 删除首页相关聚合项；系统默认分类只清空同名聚合内容，自定义分类连同跨书同名定义一起物理删除。
    func deleteRelatedCategory(scope: RelatedCategoryScope) async throws {
        ...
        try await databaseManager.database.dbPool.write { db in
            try hardDeleteRelatedCategoryAggregation(db, title: title)
        }
    }
...
private extension NoteRepository {
    ...
    nonisolated func hardDeleteRelatedCategoryAggregation(_ db: Database, title: String) throws {
        ...
        if categoryRows.contains(where: Self.isProtectedDefaultRelatedCategory) {
            let relatedBookIDs = try hardDeleteRelatedCategoryContents(db, categoryIDs: categoryIDs)
            try purgeUnreferencedRelatedBookPlaceholders(db, bookIDs: relatedBookIDs)
        } else {
            try hardDeleteRelatedCategories(db, categoryIDs: categoryIDs)
        }
    }

    /// 按已完成保护校验的分类主键执行 child-first 物理删除，并清理最后引用消失的占位书。
    nonisolated func hardDeleteRelatedCategories(_ db: Database, categoryIDs: [Int64]) throws {
        ...
        let relatedBookIDs = try hardDeleteRelatedCategoryContents(db, categoryIDs: categoryIDs)
        try db.execute(
            // SQL 目的：物理删除同名自建分类定义，自建 book_id=0 全局分类同样允许删除。
            // 涉及表：category。
            // 关键过滤：限定已确认不含固定六个系统种子分类的同名分类 ID。
            // 时间字段：不读取时间字段。
            // 副作用：删除跨书同名自定义分类，不产生 tombstone。
    ...
    /// 物理删除分类范围内的内容、附图与相关书籍关系，并返回可能需要清理的占位书主键。
    nonisolated func hardDeleteRelatedCategoryContents(_ db: Database, categoryIDs: [Int64]) throws -> [Int64] {
        guard !categoryIDs.isEmpty else { throw NoteCollectionRepositoryError.relatedCategoryNotFound }
        let placeholders = Self.placeholders(count: categoryIDs.count)
        // SQL 目的：在删除关系前锁定相关书籍占位 ID，供最后引用清理。
        // 涉及表：category_content。
        // 关键过滤：限定待删除分类且 content_book_id>0；包含历史无效关系以便完整清理。
        // 时间字段：不读取时间字段。
        // 返回字段用途：后续逐本判断占位书是否仍有引用。
        let relatedBookIDs = try Int64.fetchAll(
            db,
            sql: "SELECT DISTINCT content_book_id FROM category_content WHERE category_id IN (\(placeholders)) AND content_book_id > 0",
            arguments: StatementArguments(categoryIDs)
        )
        // SQL 目的：锁定待删除分类下所有内容主键，供 category_image child-first 删除。
        // 涉及表：category_content。
        // 关键过滤：限定待删除分类 ID 集合，包含有效记录与历史 tombstone。
        // 时间字段：不读取时间字段。
        // 返回字段用途：构建附图子表的批量删除范围。
        let contentIDs = try Int64.fetchAll(
            db,
            sql: "SELECT id FROM category_content WHERE category_id IN (\(placeholders))",
            arguments: StatementArguments(categoryIDs)
        )
        if !contentIDs.isEmpty {
            try db.execute(
                // SQL 目的：child-first 物理删除分类内容附图。
                // 涉及表：category_image。
                // 关键过滤：限定即将删除的 category_content 主键集合，不区分历史 is_deleted 状态。
                // 时间字段：不读取或换算时间字段。
                // 副作用：附图行先于父级内容物理移除，不创建 tombstone。
                sql: "DELETE FROM category_image WHERE category_content_id IN (\(Self.placeholders(count: contentIDs.count)))",
                arguments: StatementArguments(contentIDs)
            )
        }
        try db.execute(
            // SQL 目的：物理删除分类范围内全部相关内容与相关书籍关系。
            // 涉及表：category_content。
            // 关键过滤：限定精确标题解析出的分类 ID 集合，不写 tombstone。
            // 时间字段：不读取或换算时间字段。
            // 副作用：普通相关内容与相关书籍关系一并物理移除。
            sql: "DELETE FROM category_content WHERE category_id IN (\(placeholders))",
            arguments: StatementArguments(categoryIDs)
        )
        return relatedBookIDs
    }

    /// 清理相关关系移除后失去最后真实引用的业务占位书。
    nonisolated func purgeUnreferencedRelatedBookPlaceholders(_ db: Database, bookIDs: [Int64]) throws {
        for bookID in bookIDs {
            try db.execute(
                // SQL 目的：删除失去最后一个真实引用的业务占位书。
                // 涉及表：book，反查 category_content 与 collection_book。
                // 关键过滤：只处理 is_deleted=1 占位书；任何物理关系仍存在时保留，避免误删有效书或共享占位书。
                // 时间字段：不读取时间字段。
                // 副作用：最后引用移除后物理删除 book 主记录。
                sql: """
                    DELETE FROM book
...
private extension NoteRepository {
    ...
    nonisolated func replaceNoteTagAssociations(
        ...
        try db.execute(
            // SQL 目的：物理删除指定书摘的现有标签关系，随后按当前选择重建唯一有效关系。
            // 表关系：tag_note。
            // 关键过滤：按 note_id 精确命中，不区分历史 tombstone，防止替换后残留重复关系。
            // 时间字段：删除不写时间戳；新关系 created_date 使用调用方传入毫秒时间戳。
            // 副作用：删除全部旧关系，仍处于外层书摘保存事务中。
    ...
    nonisolated func replaceNoteImages(
        ...
        try db.execute(
            // SQL 目的：物理删除指定书摘的现有附图，随后按当前展示顺序重建附图记录。
            // 表关系：attach_image。
            // 关键过滤：按 note_id 精确命中，不区分历史 tombstone，避免编辑后遗留无效图片记录。
            // 时间字段：删除不写时间戳；新图片 created_date 使用调用方传入毫秒时间戳。
            // 副作用：删除全部旧附图，仍处于外层书摘保存事务中。
...
extension NoteRepository {
    ...
    /// 物理批量删除书摘；调用任务在事务开始前可取消，事务开始后按 child-first 原子完成且不留下 tombstone。
    ...
    /// 全量替换多条书摘的标签；调用任务在事务开始前可取消，全部关系使用物理删除后重建。
...
private extension NoteRepository {
    ...
    nonisolated func hardDeleteNotes(_ db: Database, noteIDs: [Int64]) throws {
        ...
        let arguments = StatementArguments(noteIDs)
        try db.execute(
            // SQL 目的：物理删除选中书摘的全部附图，解除 attach_image.note_id 外键。
            // 涉及表：attach_image -> note。
            // 关键过滤：按 note_id 集合精确命中，包括历史 tombstone，避免主记录删除后留下孤儿。
            // 时间字段：硬删除不写 updated_date。
            // 副作用：child-first 第一步，仍处于调用方写事务。
            sql: "DELETE FROM attach_image WHERE note_id IN (\(placeholders))",
            arguments: arguments
        )
        try db.execute(
            // SQL 目的：物理删除选中书摘的全部标签关系，解除 tag_note.note_id 外键。
            // 涉及表：tag_note -> note/tag。
            // 关键过滤：按 note_id 集合精确命中，包括历史 tombstone。
            // 时间字段：硬删除不写 updated_date。
            // 副作用：child-first 第二步，标签主记录保持不变。
            sql: "DELETE FROM tag_note WHERE note_id IN (\(placeholders))",
            arguments: arguments
        )
        try db.execute(
            // SQL 目的：物理删除选中书摘主记录，落实全局硬删除规则。
            // 涉及表：note；附图和标签关系已在前两步解除。
            // 关键过滤：按 id 集合精确命中，不创建 is_deleted tombstone。
            // 时间字段：硬删除不写 updated_date。
            // 副作用：删除后所有观察流在事务提交时统一刷新。

...
nonisolated struct BookWorkspacePresentationInput: Hashable, Sendable {
    ...
    let relatedSort: BookContentSortRule
    let reviewSort: BookContentSortRule
...
final class BookWorkspacePresentationStore {
    ...
    func update(with input: BookWorkspacePresentationInput) {
        ...
        let relatedChanged = previous.map {
            $0.related != input.related
                ...
                || $0.relatedSort != input.relatedSort
        } ?? true
        if relatedChanged {
            relatedRevision &+= 1
            scheduleRelated(
    ...
    nonisolated private static func makeNotesSnapshot(
        ...
        let notes = input.notes.filter { note in
        ...
        if input.notesSort == .createdDateAscending || input.notesSort == .createdDateDescending {
                ...
                sections: [BookWorkspacePresentationSnapshot.chromeSection(),
                    BookWorkspaceCollectionSectionModel(
                        ...
                        header: BookWorkspaceCollectionHeader(
                            title: input.notesSort.title(for: .notes),
                            count: notes.count,
                            isStarred: false,
                            isPinned: false
                        ),
        ...
        var orderedChapterIDs: [Int64] = []
        var seenChapterIDs: Set<Int64> = []
        for note in notes {
            let chapterID = validChapterIDs.contains(note.chapterID) ? note.chapterID : 0
            if seenChapterIDs.insert(chapterID).inserted {
                orderedChapterIDs.append(chapterID)
            }
        }
        let sections = orderedChapterIDs.compactMap { chapterID -> BookWorkspaceCollectionSectionModel? in
            guard let children = grouped[chapterID] else { return nil }

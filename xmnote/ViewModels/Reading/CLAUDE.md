...
在读模块 ViewModel 目录，承载在读首页、时间线与阅读日历的状态管理与业务编排。
...
- `TimelineViewModel.swift`: 时间线页面状态中枢（时间范围计算、首屏快照、列表加载、日历标记缓存）
- `ReadingTimerSupplementViewModel.swift`: 补记阅读日期、时长与写入反馈状态编排

## 子目录

- `ReadCalendar/`: 阅读日历子功能 ViewModel 目录

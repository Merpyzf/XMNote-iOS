 ...
 * [OUTPUT]: 对外提供 TimelineCalendarHorizonTestView（HorizonCalendar 时间线日历联动测试页）
 * [POS]: Debug 测试页，仅用于验证 Android 时间线日历特性的 iOS 端可行实现（范围/选中/月切换/跳转/marker 渲染）
...
/// HorizonCalendar 时间线日历联动测试页（仅 Debug 使用）。
...
private extension TimelineCalendarHorizonTestView {
        ...
        CardContainer {
            HStack(spacing: Spacing.base) {
                ...
                HStack(spacing: Spacing.half) {
                    ...
                    Button("指定日期") {
                        let date = calendar.date(byAdding: .day, value: -45, to: calendar.startOfDay(for: Date())) ?? Date()
                        jumpToDate(date, animated: true)
                    }
...
private extension TimelineCalendarHorizonTestView {
    var rangeText: String {
        ...
        return "\(formatter.string(from: visibleDateRange.lowerBound)) ~ \(formatter.string(from: visibleDateRange.upperBound))"
    }
    ...
    func jumpToDate(_ date: Date, animated: Bool) {
        ...
        let monthStart = Self.monthStart(of: clamped, using: calendar)
        applyDisplayedMonth(monthStart, animated: false)
        ...
        ZStack {
            if isSelected {
                ...
                if marker.readingProgress > 0 {
                    ZStack {
                        Circle()
                            .stroke(Color.brand.opacity(0.18), lineWidth: 2.2)
                        Circle()
        ...
        let allEvents: [TimelineEvent] = [
            TimelineEvent(
                ...
                kind: .note(TimelineNoteEvent(
                    content: "通过迭代验证，HorizonCalendar 已覆盖 Android 时间线日历的核心交互。",
                    idea: "先在 Debug 路径验证，收敛后再进正式页面。",
                    bookTitle: "架构演进实践"
                )),
            ...
            TimelineEvent(
                ...
                kind: .review(TimelineReviewEvent(
                    title: "本次实现点评",
                    content: "当前验证路径已打通：日期选择、月份切换、跳转与 marker 渲染均可用。",
                    bookScore: 45
                )),

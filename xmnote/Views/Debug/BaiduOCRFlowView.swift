#if DEBUG
/**
 * [INPUT]: 依赖 OCRRepositoryProtocol 与 BaiduOCRFlowViewModel 驱动 OCR Flow 状态，依赖 AVFoundation/PhotosUI 提供拍照与选图能力
 * [OUTPUT]: 对外提供 BaiduOCRFlowView，承载 Debug OCR 的相机、裁切识别与设置三段式流程
 * [POS]: Debug 模块百度 OCR 全屏 Flow，负责对齐 Android 主流程并补充 iOS 的自由框选交互
...
struct BaiduOCRFlowView: View {
    let target: OCRFlowTarget
    let repository: any OCRRepositoryProtocol
    let onComplete: (BaiduOCRFlowCompletionPayload) -> Void

    @Environment(\.dismiss) private var dismiss
    @State private var viewModel: BaiduOCRFlowViewModel
    ...
    init(
        ...
        onComplete: @escaping (BaiduOCRFlowCompletionPayload) -> Void
    ) {
        self.target = target
        self.repository = repository
        self.onComplete = onComplete
        _viewModel = State(initialValue: BaiduOCRFlowViewModel(target: target, repository: repository))
    }
    ...
    var body: some View {
        NavigationStack(path: $path) {
            ...
            .navigationDestination(for: OCRFlowRoute.self) { route in
                ...
                case .settings:
                    OCRSettingsScreen(viewModel: viewModel)
                }
            }
        }
    ...
    private func handleRecognitionCompleted(_ payload: BaiduOCRFlowCompletionPayload) {
        onComplete(payload)
        dismiss()
    }
}
...
private struct OCRCameraScreen: View {
    ...
    @Bindable var viewModel: BaiduOCRFlowViewModel
    @ObservedObject var cameraController: OCRCameraSessionController
    let onClose: () -> Void
    let onOpenSettings: () -> Void
    let onSelectedImage: (UIImage, String) -> Void

    @AppStorage("debug.baidu_ocr.camera_tip_count") private var cameraTipDisplayCount = 0
...
private struct OCRCropRecognitionScreen: View {
    ...
    let onBack: () -> Void
    let onOpenSettings: () -> Void
    let onRecognized: (BaiduOCRFlowCompletionPayload) -> Void
    ...
    var body: some View {
        ...
        .onChange(of: viewModel.selectionMode) { _, newValue in
...
private extension OCRCropRecognitionScreen {
    @ViewBuilder
    func compactTopBarIcon(_ systemName: String) -> some View {
    ...
    var moreMenu: some View {
        Menu {
            Picker("框选方式", selection: selectionModeBinding) {
                Text("选择框").tag(OCRSelectionMode.single)
                Text("自由框选").tag(OCRSelectionMode.freeform)
            }

            Divider()

            Button {
                presentInstruction(.manual)
            } label: {
                menuRow("使用说明", systemImage: "questionmark.circle")
            }

            Button {
                onOpenSettings()
            } label: {
                menuRow("OCR 设置", systemImage: "slider.horizontal.3")
            }
        } label: {
            compactTopBarIcon("ellipsis")
        }
        .buttonStyle(.plain)
        .tint(Self.darkForegroundPrimary)
    ...
    @ViewBuilder
    func menuRow(_ title: LocalizedStringKey, systemImage: String) -> some View {
    ...
    var languageMenu: some View {
        Menu {
            ...
            compactTopBarIcon(languageMenuSymbolName)
        }
        .buttonStyle(.plain)
        .tint(Self.darkForegroundPrimary)
        .accessibilityLabel("识别语言")
        .accessibilityValue(viewModel.preferences.language.title)
    }

    var languageBinding: Binding<OCRLanguage> {
        Binding(
    ...
    var languageMenuSymbolName: String {
        let preferredSymbol: String = switch viewModel.preferences.language {
        case .chnEng:
            "character.zh"
        case .eng, .por, .fre, .ger, .ita, .spa, .rus:
            "character"
        case .jap:
            "character.ja"
        case .kor:
        ...
        return availableSymbolName(
            preferred: preferredSymbol,
            fallbacks: ["character", "globe"]
        )
    }

    func availableSymbolName(preferred: String, fallbacks: [String]) -> String {
        if UIImage(systemName: preferred) != nil {
            return preferred
        }
        for fallback in fallbacks where UIImage(systemName: fallback) != nil {
            return fallback
        }
        return "globe"
    }

    var selectionModeBinding: Binding<OCRSelectionMode> {
        Binding(
            get: { viewModel.selectionMode },
            set: { updateSelectionMode($0) }
        )
    }
    ...
    func applySelectionMode(_ newValue: OCRSelectionMode) {
        switch newValue {
        case .single:
            viewModel.clearFreeformRegions()
            viewModel.clearSingleSelection()
        case .freeform:
            viewModel.clearSingleSelection()
            viewModel.selectFreeformRegion(id: nil)
        }
        presentInstruction(.automatic)
    }

    func updateSelectionMode(_ mode: OCRSelectionMode) {
        guard viewModel.selectionMode != mode else {
            presentInstruction(.manual)
            return
        }
...
private struct OCRCropRecognitionPageState {
    ...
    let freeformRegions: [OCRSelectionRegion]
...
private final class OCRSelectionCanvasView: UIView {
    ...
    private var selectionMode: OCRSelectionMode = .single
    private var singleSelectionRect: CGRect?
    private var freeformRegions: [OCRSelectionRegion] = []
...
private extension OCRSelectionCanvasView {
    ...
    func finishFreeformTouch(at location: CGPoint, in imageFrame: CGRect) {
        ...
        let region = OCRSelectionRegion(normalizedRect: normalized)
    ...
    func region(containing point: CGPoint, in imageFrame: CGRect) -> OCRSelectionRegion? {
        freeformRegions
            .reversed()
            .first { region in
...
private struct OCRSelectionEditor: View {
    ...
    @Binding var freeformRegions: [OCRSelectionRegion]

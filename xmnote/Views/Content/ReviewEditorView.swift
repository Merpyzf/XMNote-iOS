...
struct ReviewEditorView: View {
    ...
    init(mode: ReviewEditorMode, onSaved: @escaping (Int64) -> Void = { _ in }) {
        self.mode = mode
        self.onSaved = onSaved
    }

    /// 兼容既有只传书评主键的编辑路由。
    init(reviewId: Int64, onSaved: @escaping (Int64) -> Void = { _ in }) {
        self.init(mode: .edit(reviewID: reviewId), onSaved: onSaved)
    }

    /// 为书籍详情或书评列表提供明确的新建入口。
    init(bookId: Int64, onSaved: @escaping (Int64) -> Void = { _ in }) {
        self.init(mode: .create(bookID: bookId), onSaved: onSaved)
    }

    var body: some View {
        ZStack {
            if let viewModel {
                ReviewEditorLoadedView(viewModel: viewModel) { reviewID in
                    onSaved(reviewID)
                    dismiss()
                }
            } else {

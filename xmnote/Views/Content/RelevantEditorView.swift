...
struct RelevantEditorView: View {
    ...
    init(mode: RelevantEditorMode, onSaved: @escaping (Int64) -> Void = { _ in }) {
        self.mode = mode
        self.onSaved = onSaved
    }

    /// 兼容既有只传相关内容主键的编辑路由。
    init(contentId: Int64, onSaved: @escaping (Int64) -> Void = { _ in }) {
        self.init(mode: .edit(contentID: contentId), onSaved: onSaved)
    }
    ...
    var body: some View {
        ZStack {
            if let viewModel {
                RelevantEditorLoadedView(viewModel: viewModel) { contentID in
                    onSaved(contentID)
                    dismiss()
                }
            } else {

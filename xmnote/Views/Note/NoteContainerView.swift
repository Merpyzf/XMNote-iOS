 ...
 * [OUTPUT]: 对外提供 NoteContainerView 与 NoteSubTab 枚举，并上抛携带真实章节标题的笔记路由、书籍/目录定位、内容编辑及统一内容查看路由，同时提供新增优先、排序/回顾更多收尾的首页顶部操作
    ...
    private func segmentedPage(for tab: NoteSubTab) -> some View {
        ...
        case .notes:
            NoteCollectionView(
                ...
                onEditReview: {
                    navigationCoordinator.present(.reviewEditor(.edit(reviewID: $0)))
                }
            )
                .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .top)
        case .review:

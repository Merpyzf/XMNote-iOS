/**
 * [INPUT]: 依赖 SwiftUI、ReadingStatusPresentation、DesignTokens 排版/间距令牌与 CardContainer 内容表层
 * [OUTPUT]: 对外提供 ReadingStatusTimeline 核心时间线、Status/Item 输入模型与 ReadingStatusTimelineCard 卡片封装，并以单一虚线路径绘制相邻状态连接边
 * [POS]: Views/Book/Components 的页面私有阅读历程组件，仅服务单书阅读详情及其局部展示
    ...
    enum Status: Int64, CaseIterable, Hashable, Sendable {
        ...
        var tint: Color {
            ...
            case .addedToShelf, .unknown:
                .textHint
            case .wantRead:
                ReadingStatusPresentation.wantRead
            case .reading:
                ReadingStatusPresentation.reading
            case .readDone:
                ReadingStatusPresentation.readDone
            case .abandoned:
                ReadingStatusPresentation.abandoned
            case .onHold:
                ReadingStatusPresentation.onHold
            }
        }
...
private enum ReadingStatusTimelineMetrics {
    ...
    static let emptyStateMinimumHeight: CGFloat = 44
}

/// 时间线局部反馈的动画语义，只负责状态行按压，不作为跨组件 Motion token。
private enum ReadingStatusTimelineMotion {
    static let rowPress = Animation.smooth(duration: 0.14)
}
...
private struct ReadingStatusTimelineRowButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .animation(
                reduceMotion ? nil : ReadingStatusTimelineMotion.rowPress,
...
private struct ReadingStatusTimelineEmptyView: View {
    ...
    var body: some View {
        ...
        .frame(
            maxWidth: .infinity,
            minHeight: ReadingStatusTimelineMetrics.emptyStateMinimumHeight,
            alignment: .center
        )
        .accessibilityElement(children: .combine)
    }

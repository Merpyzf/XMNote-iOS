 ...
 * [OUTPUT]: 对 BookPicker Apple 推荐分支提供纯搜索用途的系统 minimal 搜索栏，并保持 UIKit 焦点生命周期与页面状态一致
 * [POS]: Book 模块页面私有 UIKit 桥接，仅服务仍在验证中的 Apple 推荐选书样式
...
/// Apple 推荐选书样式的原生搜索栏；系统负责背景、圆角、图标、清除按钮与输入交互。
@MainActor
struct BookPickerSystemSearchBar: UIViewRepresentable {
    ...
    /// 创建系统 minimal 搜索栏，并保留系统背景、圆角、图标、清除按钮与输入交互。
    func makeUIView(context: Context) -> UISearchBar {
        ...
        searchBar.searchTextField.accessibilityIdentifier = "book.picker.search"
        return searchBar
    }

    /// 只同步确实变化的外部值；第一响应者切换延后执行，避免在 SwiftUI 更新期重入 UIKit。
    ...
    final class Coordinator: NSObject, UISearchBarDelegate {
        var parent: BookPickerSystemSearchBar
        private var focusRequestID = UUID()

        /// 保存初始组件配置，后续由 updateUIView 替换为最新值。
        init(parent: BookPickerSystemSearchBar) {
            self.parent = parent
        }

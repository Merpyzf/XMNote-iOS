/**
 * [INPUT]: 依赖 SwiftUI 视图布局与 glassEffect 能力，依赖 XMBookCoverAppearance 提供进度条轨道/完成段/描边外观
 * [OUTPUT]: 对外提供 BookCoverProgressBar（书籍封面底部悬浮阅读进度条）
 * [POS]: Views/Reading/Components 的页面私有封面进度组件，仅服务阅读仪表盘书籍卡片
...
private extension BookCoverProgressBar {
    func progressBar(width: CGFloat, height: CGFloat) -> some View {
        ZStack(alignment: .leading) {
            Capsule()
                .fill(XMBookCoverAppearance.Progress.track)

            Capsule()
                .fill(XMBookCoverAppearance.Progress.fill)
                .frame(width: width * clampedProgress)
        }
        .frame(height: height)
        .overlay {
            Capsule()
                .stroke(XMBookCoverAppearance.Progress.stroke, lineWidth: Layout.borderWidth)
        }

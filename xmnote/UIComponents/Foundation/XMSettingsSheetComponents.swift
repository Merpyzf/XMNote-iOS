/**
 * [INPUT]: 依赖 DesignTokens、TopBarActionIcon、XMScrollEdgeChrome 与 SwiftUI Menu/Toggle/Button 等系统控件，接收字符串或自定义副标题及可选关闭按钮视觉尺寸
 * [OUTPUT]: 对外提供支持自定义副标题、固定内容顶部栏、底部栏、局部关闭按钮尺寸和可配置系统滚动边缘语义的 XMSettingsPageScaffold，以及 XMSettingsGroupCard、内容自适应选项胶囊、弱分割线与通用设置行组件
...
struct XMSettingsPageScaffold<Content: View, SubtitleContent: View, ContentTopBar: View, BottomBar: View>: View {
    let title: String
    let subtitle: String?
    let onClose: () -> Void
    let content: Content
    ...
    private let showsCustomSubtitle: Bool
    ...
    init(
    ...
    ) where SubtitleContent == EmptyView, ContentTopBar == EmptyView, BottomBar == EmptyView {
        self.title = title
        ...
        self.showsCustomSubtitle = false
    }
    ...
    init(
        ...
        @ViewBuilder content: () -> Content
    ) where SubtitleContent == EmptyView, ContentTopBar == EmptyView {
        self.title = title
        self.subtitle = subtitle
        self.onClose = onClose
        self.content = content()
    ...
    init(
        ...
        @ViewBuilder content: () -> Content
    ) where SubtitleContent == EmptyView, BottomBar == EmptyView {
        self.title = title
        self.subtitle = subtitle
        self.onClose = onClose
        self.content = content()
    ...
    /// 注入自定义副标题、固定内容顶部栏与底部栏；字符串副标题调用保持原有类型推断与布局行为。
    init(
        title: String,
        onClose: @escaping () -> Void,
        closeVisualSize: CGFloat = XMSettingsSheetLayout.closeVisualSize,

...
/// 标签选择器的可选目录管理能力，把适用范围、真实写入仓储和页面缓存回写绑定为一个原子配置。
struct XMTagSelectionManagementConfiguration {
    ...
    /// 为指定标签范围启用编辑和删除；成功事件由调用方同步其已有页面快照。
    init(
        scope: TagManagementScope,
        repository: any TagManagementRepositoryProtocol,
...
struct XMTagSelectionSheet: View {
    ...
    let layoutPreferenceRepository: any TagSelectionLayoutPreferenceRepositoryProtocol
    ...
    init(
        ...
        layoutPreferenceRepository: any TagSelectionLayoutPreferenceRepositoryProtocol,
        ...
        self.layoutPreferenceRepository = layoutPreferenceRepository
        ...
        _layoutMode = State(initialValue: layoutPreferenceRepository.fetchLayoutMode())
    }
        ...
        case .rename(let item):
            XMTagNameSheet(
                ...
                onSubmit: { name in
                    ...
                    try await management.repository.updateTag(
                        tagID: item.id,
                        name: name,
    ...
    private func updateLayoutMode(_ newLayoutMode: TagSelectionLayoutMode) {
        ...
        layoutPreferenceRepository.saveLayoutMode(newLayoutMode)
    ...
    private func deleteItem(_ item: XMTagSelectionItem) {
        ...
        deleteTask = Task {
            do {
                try await management.repository.deleteTags(
                    tagIDs: [item.id],
                    scope: management.scope
                )

/**
 * [INPUT]: 依赖 XMBookCover 渲染书籍封面，依赖 DesignTokens 提供颜色、圆角与描边语义
 * [OUTPUT]: 对外提供 XMBookGroupCover，用紧凑书籍叠放、书盒、规整裁片或轻托盘样式表达书籍分组封面
 * [POS]: UIComponents/Foundation 的跨模块分组封面组件，被书籍分组、书架等聚合入口复用
...
struct XMBookGroupCover: View {
    ...
    private var compactListCover: some View {
        ...
        .overlay {
            coverShape.stroke(Color.surfaceBorderSubtle.opacity(0.68), lineWidth: CardStyle.borderWidth)
        }
    ...
    private var collectionCaseCompactCover: some View {
        ...
        .overlay {
            outerShape.stroke(Color.surfaceBorderSubtle.opacity(0.54), lineWidth: CardStyle.borderWidth)
        }
        .accessibilityHidden(true)
    }
    ...
    private var orderedGridCompactCover: some View {
        ...
        return ZStack(alignment: .topLeading) {
            ...
            .clipShape(gridShape)
            .overlay {
                gridShape.stroke(Color.surfaceBorderSubtle.opacity(0.38), lineWidth: CardStyle.borderWidth)
            }
        ...
        .clipShape(outerShape)
        .overlay {
            outerShape.stroke(Color.surfaceBorderSubtle.opacity(0.50), lineWidth: CardStyle.borderWidth)
        }
        .accessibilityHidden(true)
    }
    ...
    private func adaptiveTray(size: CGSize, origin: CGPoint, cornerRadius: CGFloat) -> some View {
        ...
        return shape
            ...
            .overlay {
                shape.stroke(Color.surfaceBorderSubtle.opacity(0.22), lineWidth: CardStyle.borderWidth)
            }
    ...
    private func groupCoverSlot(_ slot: XMBookGroupCoverSlot) -> some View {
        ...
        return XMBookCover.fixedSize(
            ...
            border: .init(color: slot.borderColor.opacity(slot.borderOpacity), width: CardStyle.borderWidth),
        ...
        .shadow(color: Color.bookCoverDropShadow.opacity(slot.shadowOpacity), radius: slot.shadowRadius, x: 0, y: slot.shadowY)
    ...
    private func collectionCaseSlot(_ slot: XMBookGroupCollectionCaseSlot) -> some View {
        ...
        .shadow(color: Color.bookCoverDropShadow.opacity(slot.shadowOpacity), radius: slot.shadowRadius, x: slot.shadowX, y: slot.shadowY)
        .position(
    ...
    private func adaptiveBookCoverCell(
        ...
        border: XMBookCover.Border? = .init(color: Color.surfaceBorderSubtle.opacity(0.50), width: CardStyle.borderWidth),
        ...
        .shadow(color: Color.bookCoverDropShadow.opacity(shadowOpacity), radius: shadowRadius, x: 0, y: shadowY)
        .position(x: origin.x + size.width / 2, y: origin.y + size.height / 2)
    }

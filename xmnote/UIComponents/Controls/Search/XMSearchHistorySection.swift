/**
 * [INPUT]: 依赖搜索历史词数组、展开/编辑状态绑定、编辑态展开策略与按下/取消按下/点击/删除/清空回调，依赖 DesignTokens、组件私有胶囊外观、SwiftUI Layout 与可选 Liquid Glass 效果
 ...
 * [POS]: UIComponents/Controls/Search 的通用搜索历史组件，被全局搜索、书籍搜索与 Debug 测试页复用
...
struct XMSearchHistorySection: View {
    ...
    private var toggleRow: some View {
        HStack {
            ...
            .background(XMSearchHistoryAppearance.toggleBackground.opacity(0.92), in: Capsule())
            .overlay {
                Capsule()
                    .stroke(Color.surfaceBorderSubtle, lineWidth: StrokeWidth.hairline)
            }
    ...
    private func layoutItem(for query: String, maximumChipWidth: CGFloat?) -> XMSearchHistoryChipLayoutItem {
        let font = XMSearchHistoryTypography.uiChip
...
private struct XMSearchHistoryChip: View {
    ...
    private var removeButtonOffset: CGFloat {
        let hitExpansion = (
            XMSearchHistoryMetrics.removeButtonHitWidth
                - XMSearchHistoryMetrics.removeButtonVisualSlotWidth
        ) / 2
        let trailingPosition = max(
            visualWidth - XMSearchHistoryMetrics.removeButtonVisualSlotWidth - hitExpansion,
            0
        )
    ...
    private var queryLabel: some View {
        Text(query)
            .font(XMSearchHistoryTypography.chip)
            .foregroundStyle(Color.textSecondary)
            .lineLimit(1)
            .truncationMode(.tail)
...
/// 搜索历史胶囊排版入口：统一渲染字体与宽度测量字体。
private enum XMSearchHistoryTypography {
    static let chip: Font = AppTypography.footnoteMedium

    static var uiChip: UIFont {
        AppTypography.uiSemantic(.footnote, weight: .medium)
    }
}
...
private enum XMSearchHistoryMetrics {
    ...
    static let minimumChipHitWidth: CGFloat = InteractionMetrics.minimumTouchTarget
    ...
    static let removeButtonVisualSlotWidth: CGFloat = 32
    static let removeButtonHitWidth: CGFloat = InteractionMetrics.minimumTouchTarget
    static let removeButtonCollapsedDrift: CGFloat = 8
    static let headerActionMinWidth: CGFloat = InteractionMetrics.minimumTouchTarget
    static let headerActionTapHeight: CGFloat = InteractionMetrics.minimumTouchTarget
...
/// 搜索历史展开操作的私有外观，不将交互胶囊误归类为领域标签。
private enum XMSearchHistoryAppearance {
    static let toggleBackground = Color.xmAdaptive(

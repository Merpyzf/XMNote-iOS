 ...
 * [POS]: UIComponents/Controls/Selection 的范围选择控件，服务搜索范围、结果来源与显示模式等跨页面可复用场景
...
struct XMScopeSelector<ID: Hashable>: View {
    ...
    private var shellBorder: some View {
        Capsule()
            .stroke(shellStroke, lineWidth: StrokeWidth.hairline)
            .frame(height: visualHeight)
    }
    ...
    private func selectedIndicator(controlWidth: CGFloat) -> some View {
        Capsule()
            ...
            .overlay {
                Capsule()
                    .stroke(selectionStroke, lineWidth: StrokeWidth.hairline)
            }
            .shadow(
    ...
    private func countBadgeFill(isSelected: Bool) -> Color {
        if isSelected {
            return Color.selectionAccent.opacity(0.10)
        }
    ...
    private var selectedCountColor: Color {
        switch style {
        case .content:
            return Color.selectionForeground.opacity(0.82)
        case .floatingGlass:
            return Color.selectionForeground.opacity(0.78)
        }
    }

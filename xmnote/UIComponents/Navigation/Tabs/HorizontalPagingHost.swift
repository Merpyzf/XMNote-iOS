 ...
 * [POS]: UIComponents/Navigation/Tabs 的通用横向分页宿主，被阅读日历与内容查看页复用
 * [PROTOCOL]: 变更时更新此头部，然后检查 CLAUDE.md
 */

import SwiftUI

/// 横向分页的局部运动语义，保持程序化翻页与既有交互节奏一致。
enum HorizontalPagingMotion {
    static let programmaticScroll = Animation.snappy(duration: 0.24)
}
...
struct HorizontalPagingHost<ID: Hashable & Sendable, PageContent: View>: View {
    ...
    var programmaticScrollAnimation: Animation = HorizontalPagingMotion.programmaticScroll
    ...
    init(
        ...
        programmaticScrollAnimation: Animation = HorizontalPagingMotion.programmaticScroll,

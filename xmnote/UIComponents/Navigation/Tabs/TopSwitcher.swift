/**
 * [INPUT]: 依赖 UIComponents/Navigation/TopBar 的 PrimaryTopBar 与 AddMenuCircleButton，依赖 SwiftUI 事务、局部动画与无障碍能力
 * [OUTPUT]: 对外提供 TopSwitcher 组件（支持标签模式与标题模式，默认将外部 selection 作为无动画路由写入）
 * [POS]: UIComponents/Navigation/Tabs 的顶部切换入口，被 Book/Note/Reading/Personal 页面复用
...
private enum TopSwitcherTypography {
    ...
    static let selectedTab: Font = AppTypography.fixed(
        baseSize: 20,
        relativeTo: .title3,
        weight: .semibold,
        minimumPointSize: 20
    )
    static let unselectedTab: Font = AppTypography.fixed(
        baseSize: 18,
        relativeTo: .title3,
        weight: .medium,
        minimumPointSize: 18
    ...
    static func titleFont(for text: String) -> Font {
        if containsCJK(text) {
            return AppTypography.fixed(
                baseSize: titleSize,
                relativeTo: .headline,
                weight: .semibold,
                minimumPointSize: titleSize
            )
        }
        return AppTypography.brandDisplay(size: titleSize, relativeTo: .headline)
    }

    static func titleTrim(for text: String) -> BrandTypography.VerticalTrim {
        guard !containsCJK(text) else { return .zero }
        return AppTypography.brandTrim(size: titleSize, textStyle: .headline)
    }
...
private struct TopSwitcherTabBar<Tab: Hashable>: View {
    ...
    private func tabItem(_ tab: Tab) -> some View {
        ...
        return Button {
            ...
            Text(title)
                .font(isSelected ? TopSwitcherTypography.selectedTab : TopSwitcherTypography.unselectedTab)
...
private struct TopSwitcherTitleLabel: View {
    ...
    private var titleTrim: BrandTypography.VerticalTrim {
        TopSwitcherTypography.titleTrim(for: text)
    }

    var body: some View {
        Text(text)
            .font(TopSwitcherTypography.titleFont(for: text))

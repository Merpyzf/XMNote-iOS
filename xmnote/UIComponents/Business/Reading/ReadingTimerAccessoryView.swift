 ...
 * [POS]: UIComponents/Business/Reading 的跨 Tab 计时状态条，只负责展示与高频可逆操作，不持有业务状态或计时生命周期
    ...
    private func inlineContent(seconds: Int64) -> some View {
        HStack(spacing: Spacing.cozy) {
            Button(action: onOpen) {
                ...
                .frame(
                    maxWidth: .infinity,
                    minHeight: ReadingTimerAccessoryLayout.inlineHeight,
                    maxHeight: ReadingTimerAccessoryLayout.inlineHeight,
                    alignment: .leading
                )
                .contentShape(Rectangle())
            }
...
private enum ReadingTimerAccessoryLayout {
    ...
    static let inlineHeight: CGFloat = InteractionMetrics.minimumTouchTarget

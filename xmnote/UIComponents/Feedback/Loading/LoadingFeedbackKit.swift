 ...
 * [POS]: UIComponents/Feedback/Loading 的加载反馈基础设施，统一“延迟显示 + 最短驻留 + 阶段承载”模式
...
#Preview("加载门闩") {
    @Previewable @State var gate = LoadingGate()
    ...
    VStack(spacing: Spacing.base) {
        ...
        HStack(spacing: Spacing.base) {
            Button("读取加载") {
                gate.update(intent: .read)
            }
            Button("停止") {
                gate.update(intent: .none)
            }
        }
...
#Preview("阶段宿主") {
    VStack(spacing: Spacing.base) {
        LoadPhaseHost(
            phase: .loading,
            content: { Text("内容") },
            placeholder: { Color.clear.frame(height: 24) },
            loading: { LoadingStateView("正在加载") },
            empty: { EmptyStateView(icon: "tray", message: $0) },
            failure: { Text($0).foregroundStyle(Color.feedbackError) }
        )

        LoadPhaseHost(
            phase: .error(message: "加载失败"),
            content: { Text("内容") },
            placeholder: { Color.clear.frame(height: 24) },
            loading: { LoadingStateView("正在加载") },
            empty: { EmptyStateView(icon: "tray", message: $0) },

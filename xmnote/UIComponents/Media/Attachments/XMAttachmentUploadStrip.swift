 ...
 * [POS]: UIComponents/Media/Attachments 的通用附图上传条组件，统一承接上传态展示、长按拖拽排序、删除/重试与可选全屏预览交互
...
/// 附图重试操作的排版入口，统一初始化与运行时文案更新的字体。
private enum XMAttachmentUploadTypography {
    static var retryUIFont: UIFont {
...
private final class XMAttachmentUploadCell: UICollectionViewCell {
    ...
    private let retryButton = XMMinimumHitTargetButton(type: .system)
    private let removeButton = XMMinimumHitTargetButton(type: .system)
    ...
    func updateRetryButtonTitle(_ title: String) {
        ...
        let retryFont = XMAttachmentUploadTypography.retryUIFont
...
private extension XMAttachmentUploadCell {
    ...
    func setupCell() {
        ...
        removeButton.hitTargetAnchor = .topTrailing
    ...
    func animateRemoveButton(isPressed: Bool) {
        ...
        let targetTransform: CGAffineTransform = isPressed && !reduceMotion

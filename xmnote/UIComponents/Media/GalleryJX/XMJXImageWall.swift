 ...
 * [POS]: UIComponents/Media/GalleryJX 的 SwiftUI 展示入口，负责宫格渲染与浏览器触发，并强持有 Presenter 避免 weak delegate 被提前释放
    ...
    var body: some View {
        LazyVGrid(columns: gridColumns, spacing: spacing) {
            ForEach(Array(items.enumerated()), id: \.element.id) { index, item in
                ...
                .buttonStyle(.plain)
                .accessibilityLabel(item.accessibilityLabel ?? String(localized: "图片"))
                .accessibilityValue(Text(verbatim: "\(index + 1)/\(items.count)"))
            }
        }
        .task {

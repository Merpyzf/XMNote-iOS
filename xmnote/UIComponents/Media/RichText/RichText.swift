/**
 * [INPUT]: 依赖 RichTextEditor/HTMLParser（HTML→NSAttributedString）、RichTextLayoutManager（引用块/列表绘制）与 RichTextAppearance 默认引用外观
 * [OUTPUT]: 对外提供 RichText（只读 HTML 富文本展示组件，支持 maxLines 截断、文本对齐、截断回调与可选选区菜单动作）
 * [POS]: UIComponents/Media/RichText 的跨模块只读富文本组件，供时间线卡片与笔记预览等场景使用

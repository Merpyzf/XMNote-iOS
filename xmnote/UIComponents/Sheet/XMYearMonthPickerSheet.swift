 ...
 * [POS]: UIComponents/Sheet 的年月选择组件，服务需要随机访问选择年份或年月的业务场景
...
struct XMYearMonthPickerSheet: View {
    ...
    private enum Motion {
        static let yearSelection = Animation.snappy(duration: 0.24)
    }
...
private extension XMYearMonthPickerSheet {
    ...
    var yearSelector: some View {
        ScrollViewReader { scrollProxy in
            ...
            .onChange(of: draftYear) { _, year in
                scrollToYear(year, using: scrollProxy)
            }
        }
    }
    ...
    func yearChip(_ year: Int) -> some View {
        ...
        return Button {
            ...
            updateDraftYear(year)
        } label: {
            Text(verbatim: "\(year)")
                .font(AppTypography.subheadlineSemibold)
                .foregroundStyle(isSelected ? Color.selectionForeground : Color.textPrimary)
                .monospacedDigit()
                .padding(.horizontal, Spacing.base)
                .frame(minHeight: Layout.yearChipMinHeight)
                .background(
                    Capsule()
                        .fill(isSelected ? Color.selectionAccent.opacity(0.10) : Color.controlFillSecondary.opacity(0.82))
                )
                .overlay(
                    Capsule()
                        .stroke(
                            isSelected ? Color.selectionAccent.opacity(0.30) : Color.surfaceBorderSubtle.opacity(0.72),
    ...
    func scrollToYear(_ year: Int, using proxy: ScrollViewProxy) {
        if reduceMotion {
            proxy.scrollTo(year, anchor: .center)
        } else {
            withAnimation(Motion.yearSelection) {
                proxy.scrollTo(year, anchor: .center)
            }
        }
    }

    func updateDraftYear(_ year: Int) {
        if reduceMotion {
            draftYear = year
        } else {
            withAnimation(Motion.yearSelection) {
    ...
    func currentDot(isEnabled: Bool) -> some View {
        Circle()
            .fill(isEnabled ? Color.selectionAccent : Color.textHint.opacity(0.45))
    ...
    func titleColor(isEnabled: Bool, isSelected: Bool) -> Color {
        ...
        return isSelected ? Color.selectionForeground : Color.textPrimary
    }

    func buttonBackground(isEnabled: Bool, isSelected: Bool) -> some ShapeStyle {
        if isSelected {
            return Color.selectionAccent.opacity(0.18)
        }
        if isEnabled {
    ...
    func buttonStroke(isEnabled: Bool, isSelected: Bool) -> some View {
        RoundedRectangle(cornerRadius: CornerRadius.blockSmall, style: .continuous)
            .stroke(
                isSelected
                    ? Color.selectionAccent.opacity(0.45)
                    : (isEnabled ? Color.surfaceBorderSubtle : Color.surfaceBorderSubtle.opacity(0.22)),
                lineWidth: isSelected ? Layout.selectedStrokeWidth : StrokeWidth.hairline
            )
    }

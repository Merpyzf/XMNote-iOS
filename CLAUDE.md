...
注释执行约束（强制）：新增/修改代码时，必须提供与复杂度匹配的高信息注释，优先说明“为什么这样做”；禁止无信息量注释与代码复述。
UI 组件注释执行约束（强制）：`xmnote/UIComponents` 与 `xmnote/Views/<Feature>/Components` 的新增/重大修改组件，必须在组件定义处明确“组件作用”与“所属功能/使用场景”；关键参数需说明语义、边界与空态/异常策略。
复杂 UI 分支注释约束（强制）：涉及多状态切换、布局技巧或交互约束的实现，必须补充简短分块注释标注意图，降低误改风险。

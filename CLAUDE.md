...
归因先于方案（强制）：需求明确不等于归因明确；一旦涉及行为判断、根因判断、架构判断，必须先完成归因，再决定是否实现与如何实现。
问题分层（强制）：单点现象默认按单点问题排查，禁止在证据不足时直接上升为“架构问题”“框架缺能力”或“需要基建”。
最小事实闭环（强制）：在推动任何抽象或系统性改造前，至少完成以下核对：可复现路径、真实 owner、真实写入点、生命周期/调用时机、平台事实来源（代码、最小实验或官方文档）。
抽象准入（强制）：只有当同一类问题在多个独立场景中被证明具备相同根因和相同修复模式后，才允许上升为公共方案或基础设施。
...
迁移对齐边界（强制）：Android 或其他平台经验只能帮助理解业务意图，不能直接当作当前平台事实；涉及平台行为判断时，必须回到当前平台的实际代码、最小实验或官方文档。
数据库对齐铁律（强制）：凡属于 Android → iOS 功能对齐/迁移需求，iOS 端数据库实现必须与 Android 端严格一致；覆盖范围至少包括 schema（表/列/类型/默认值）、migration 版本与执行顺序、seed 数据、外键与级联策略、事务边界、冲突策略、读写 SQL 条件（含 `is_deleted` 语义）。禁止基于主观判断自行优化、补丁式发挥或擅自变更语义。
偏离审批（强制）：如确需偏离 Android 数据库实现，必须先提交双端对照（Android/iOS 代码路径、行为差异、风险评估、回滚方案），并获得用户明确确认后方可实现与提交；未确认前一律不得落地。
澄清后执行：当需求存在高影响歧义时先澄清；若需求明确，则在完成必要事实验证后落地实现并给出可验证结果。

通用判断例子：
- 例 1：某个页面返回后状态变化，先查是谁改了状态、状态 owner 在哪，而不是先认定需要状态基建。
- 例 2：某个交互表现异常，先查页面实现和调用时机，而不是先怀疑框架不可靠。
- 例 3：某个问题只在一个场景出现，先证明它是共性，再决定是否抽公共层。
- 例 4：参考另一端平台经验时，只能借鉴业务意图，不能直接把另一端机制当作本端事实。
...
顶部图标职责约束（强制）：`TopBarActionIcon` 只用于普通顶部 action icon，不承载返回语义；若未来出现深色前景、禁用态或透明度差异，优先扩展 `TopBarBackButton` 的通用参数，禁止再派生 `GlassBackButton`、`OCRBackButton` 一类平行组件。
导航栏按钮玻璃禁令（强制）：已处于系统导航栏上下文的返回或操作按钮，禁止再显式增加 `.glassEffect(...)`、`.buttonStyle(.glass)`、`.buttonStyle(.glassProminent)` 或等价自定义 glass/material 包装；系统导航栏按钮已经由上下文提供原生视觉表达，重复叠加会直接造成双层玻璃与比例失真。
...
及时响应原则：所有异步操作（网络请求、数据库写入）必须提供可感知反馈（加载态、按钮状态切换、遮罩等），严禁出现用户点击后界面无任何响应的“假死”状态。
加载反馈分级（强制）：读取类加载采用“延迟显示 + 最短驻留”策略，默认阈值 `delay=150ms`、`minimumVisible=200ms`，用于抑制瞬时闪烁。
写操作反馈分级（强制）：保存/删除/提交等写操作必须即时反馈，不做延迟显示；操作期间必须禁用相关交互控件，防止重复触发。
加载组件边界（强制）：生产页面读取加载统一使用 `LoadingGate + LoadingStateView` 或 `LoadPhaseHost`；禁止新增裸 `ProgressView` 作为读取加载主态（写操作局部即时反馈除外）。
加载体验约束（强制）：本地查询极快场景优先无感反馈，避免默认全屏阻塞式 loading 降低视觉观感。
...
类文档注释约束（强制）：说明职责、使用位置与边界（负责什么/不负责什么）。
方法文档注释约束（强制）：至少说明业务动作、主要输入输出与调用方价值；若存在副作用、异步流程或错误语义，必须补充行为预期。
...
注释约束同构（强制）：`AGENTS.md` 与 `CLAUDE.md` 的注释约束条款必须保持语义一致。
...
语言：统一使用中文。
思考语言：技术流英文（内部推理）。
注释规范：以第五节“文档注释规范（强制）”为唯一真相源；本节仅强调中文表达与简洁准确。
...
组件文档机制（强制）：重要 UI 组件（`docs/architecture/UI核心组件白名单.md` 白名单组件 + `xmnote/UIComponents` 下新增/重大重构组件）在收到用户“任务已完成”信号后，必须新增或更新使用文档，并同步维护 `docs/architecture/UI组件文档清单.md`。
对齐情况文档机制（强制）：对 Android → iOS 迁移功能，在收到用户“任务已完成”信号后，必须在 `docs/feature/功能名/对齐情况.md` 生成或更新文档。
对齐情况文档内容规范（强制）：《对齐情况.md》必须按列表逐项罗列每项功能点，并包含 iOS 行为、Android 行为、对齐状态（已对齐/未对齐/有意差异）和双端代码证据（文件路径 + 行号）；未对齐项必须评估为“功能优化”或“设计倒退”，并给出反向指导 Android 端优化的建议。
维护原则：文档是决策记录，不是说明书。在文档收口阶段，当实现偏离文档时，优先更新文档，再改代码。
...
提交拆分要求：单次提交只做一个逻辑变更；跨模块且相互独立的改动必须拆分提交。
全量提交特例：当用户明确要求“提交本地所有改动”时允许合并提交，但提交正文必须逐条说明改动点、影响范围与验证结果。
提交正文要求：当改动涉及多个文件，或包含配置/脚本/依赖变更时，提交正文必填，至少包含 `变更点`、`影响范围`、`验证命令与结果`。
提交前自检：必须先执行 `git status --short` 与 `git diff --stat`；发现无关改动需先和用户确认是否纳入本次提交。
...
导航栏玻璃禁令（强制）：已处于系统导航栏上下文的按钮，禁止再显式增加 `.glassEffect(...)`、`.buttonStyle(.glass)`、`.buttonStyle(.glassProminent)` 或等价 glass/material 包装；系统导航栏已提供原生导航语义与视觉表达，额外叠加只会形成双层玻璃、比例失真与材质重复。
页面状态相关文档同步（强制）：当状态恢复链路或接入模式发生变更时，在文档收口阶段必须同步更新 `docs/architecture/页面状态基建与开发模式.md`。
加载状态相关文档同步（强制）：当 `LoadingIntent/LoadingPolicy/LoadingGate/LoadPhaseHost/LoadingStateView` 规则或接入模式发生变更时，在文档收口阶段必须同步更新 `docs/architecture/加载状态反馈基建设计.md`。
...
<WORKFLOW>
强制回环工作流：
前置门槛：仅当用户明确发出“任务已完成”后，启动正向流（代码→文档）；此前流程挂起且禁止文档落盘。
...
<FORBIDDEN>
禁止行为：
- FATAL-001 孤立代码变更：在用户已发出“任务已完成”后，若改代码不检查文档，回滚。
...
所有字体、间距、圆角都必须先按语义选型，再决定是否需要 token；禁止把魔法数字直接散落到生产路径。

### Typography 选择指南（五步决策）

第一步 — 先判断对象类型：

- `生产文本`：正文、标题、说明、标签、单位、表格/图表文字、按钮文字。
- `品牌数字与品牌标题`：关键数字、品牌锚点标题、日期分组锚点等需要品牌识别的强调位。
- `图标或装饰 glyph`：SF Symbol、装饰性字形、状态图标、纯 icon button。
...
- 品牌字体只负责强调，不负责承载大段信息密度。
- 数字与单位混排时，数字可用品牌字体，单位默认降回系统语义字体，保持可读性与中文一致性。
- 中文标题若品牌字体按字形回退不稳定，优先使用系统语义字体，不要强行维持品牌字体外观。

### 语义迁移后的布局处理（强制）

- 语义化后若出现显示不下，先排查容器高度、最小宽度、`lineLimit`、测量链路、布局优先级与 token 归位。
- 标题、说明、按钮文案允许通过两行、最小高度、布局重排解决，不得优先回退到固定字号。
- 禁止为了适配字号缩写业务文案、压缩“小时/分钟”等完整中文单位，或把生产文本改回硬编码 point size。
- 语义迁移的验收标准是：默认态视觉密度基本不变、Dynamic Type 能正确放大、布局链路不漂移。
...
第一步 — 判断元素角色：`inlay`（嵌入零件）→ `block`（独立单元）→ `container`（外壳容器）。
